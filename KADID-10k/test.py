...
def eval(checkpoint, test_file, test_file_path, save_file, idx=1):
    for epoch in range(3, 12):
        ...
        model.load_state_dict(torch.load('{}/{}/checkpoint/DataParallel-{:0>5d}.pt'.format(checkpoint, idx, epoch))['state_dict'])
        mask_model = Mask(model)
        mask_model.magnitudePruning(0.7)
        model.eval()
        ...
        count = 0
        with open(save_file.format(idx, epoch), 'w') as wfile:
            for line in lines:  # 将csv 文件中的数据保存到birth_data中
                ...
                predp, _ = model(img)  # 'paras' contains the network weights conveyed to target network
        
                wfile.write("{},{},{},{}\n".format(img_name, mos_value, pred.item(), predp.item()))
                count += 1
                if count % 10 == 0:
                    print(count)
...
if __name__ == '__main__':
    for idx in range(1, 11):
        ...
        test_file_path = '/home/zhihua/IQA_database/kadid10k/'
        save_file = 'preds_co_evolvo/kadid10k_test_split{}_{}.txt'
        checkpoint = 'balanced_0.7'
        save_path = os.path.join(checkpoint, 'preds_co_evolvo')
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        eval(checkpoint, test_file, test_file_path, os.path.join(checkpoint, save_file), idx) 

    # for idx in range(1, 9):
    #     test_file = 'kadid10k_splits3/{}/kadid10k_test.txt'.format(idx)
    #     test_file_path = '/home/zhihua/IQA_database/kadid10k/'
    #     save_file = 'preds/kadid10k_test_split{}.txt'.format(idx)
    #     checkpoint = 'balanced_0.65'
    #     save_path = os.path.join(checkpoint, 'preds')
    #     if not os.path.exists(save_path):
    #         os.mkdir(save_path)
    #     eval(checkpoint, test_file, test_file_path, os.path.join(checkpoint, save_file), idx) 
