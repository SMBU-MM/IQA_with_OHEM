...
class Trainer(object):
    def __init__(self, config):
        ...
        self.train_q_loss = []
        self.train_std_loss = []
        self.test_results_srcc = {'bid': [], 'clive': [], 'koniq': []}
        self.test_results_plcc = {'bid': [], 'clive': [], 'koniq': []}
    ...
    def _train_single_epoch(self, epoch):
        ...
        for step, sample_batched in enumerate(self.train_loader, 0):
            ...
            y1_p, y1_var_p = self.model(x1)
            y2_p, y2_var_p = self.model(x2)
            y_diff_p = y1_p - y2_p
        ...
        srcc, plcc = self.eval()
        self.test_results_srcc['bid'].append(srcc['bid'])
        self.test_results_srcc['clive'].append(srcc['clive'])
        self.test_results_srcc['koniq'].append(srcc['koniq'])
       
        self.test_results_plcc['bid'].append(plcc['bid'])
        self.test_results_plcc['clive'].append(plcc['clive'])
        self.test_results_plcc['koniq'].append(plcc['koniq'])

        tb = pt.PrettyTable()
        tb.field_names = ["SRCC", "BID", "LIVEC", "KONIQ"]
        tb.add_row(['SRCC', srcc['bid'], srcc['clive'], srcc['koniq']])
        tb.add_row(['PLCC', plcc['bid'], plcc['clive'], plcc['koniq']])
        print(tb)
    ...
    def eval(self):
        ...
        bid_test_loader = self._testloader(csv_file = os.path.join(config.bid_set, 'splits2', str(config.split), 'bid_test.txt'),
            img_dir = config.bid_set)
        clive_test_loader = self._testloader(csv_file = os.path.join(config.clive_set, 'splits2', str(config.split), 'clive_test.txt'),
            img_dir = config.clive_set)
        koniq_test_loader = self._testloader(csv_file = os.path.join(config.trainset, config.kadid_LT, str(config.split), 'koniq10k_test.txt'),
            img_dir = config.koniq_set)
       
        srcc['bid'], plcc['bid'] = self._eval_single(bid_test_loader)
        srcc['clive'], plcc['clive'] = self._eval_single(clive_test_loader)
        srcc['koniq'], plcc['koniq'] = self._eval_single(koniq_test_loader)
